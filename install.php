...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['folder_name'])) {
    ...
    if (empty($folder_name)) {
    ...
    } elseif ($use_directadmin && empty($db_name_raw)) {
        ...
        try {
            ...
            // Step 1: Create database in DirectAdmin when credentials are configured
            if ($use_directadmin) {
            ...
            install_site_settings_plugin($install_path);
            remove_default_plugins($install_path);

            // Step 3: Write wp-config.php when database credentials are available
            if ($use_directadmin) {
...
function directadmin_credentials_available($da_user, $da_pass) {
    return trim($da_user) !== '' && trim($da_pass) !== '';
}
...
function install_site_settings_plugin($install_path) {
    global $site_settings_plugin_url;

    $plugins_dir = $install_path . '/wp-content/plugins';
    $plugin_zip  = $plugins_dir . '/wp-site-settings.zip';

    if (!download_file($site_settings_plugin_url, $plugin_zip)) {
        throw new Exception("Failed to download site settings plugin");
    }
    ...
    if ($zip->open($plugin_zip) === TRUE) {
        ...
        $extracted = $plugins_dir . '/wp-site-settings-main';
        $target    = $plugins_dir . '/wp-site-settings';
        if (is_dir($target)) remove_directory($target);
        if (is_dir($extracted)) rename($extracted, $target);
        unlink($plugin_zip);
    } else {
        throw new Exception("Failed to extract site settings plugin");
    }
}
...
<body>
    <div class="container">
        ...
        <table>
            ...
            <?php else: ?>
            <tr><td>Database</td><td>Skipped because DirectAdmin credentials are not configured.</td></tr>
            <?php endif; ?>
        </table>
        <?php if (!empty($r['db_configured'])): ?>
        ...
        <?php else: ?>
        <div class="warn">DirectAdmin was skipped, so wp-config.php was not created. WordPress will ask for database details during setup.</div>
        <a href="<?php echo htmlspecialchars($r['wp_setup']); ?>" class="btn" target="_blank">Continue WordPress Setup</a>
        <a href="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" class="btn btn-secondary">Install Another</a>
        <?php endif; ?>
    </div>
...
$directadmin_configured = directadmin_credentials_available($da_user_default, $da_pass_default);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="container">
    ...
    <form method="post" id="installForm" autocomplete="on">
        ...
        <div class="form-group">
            ...
            <input type="text" id="db_name" name="db_name" <?php echo $directadmin_configured ? 'required' : ''; ?>
                   placeholder="e.g. 03test or democlient"
                   value="<?php echo htmlspecialchars($_POST['db_name'] ?? ''); ?>"
                   oninput="updatePreviews()">
            <div class="preview">Will create: <span id="db_preview"><?php echo $directadmin_configured ? htmlspecialchars($da_user_default . '_dbname') : 'DirectAdmin disabled'; ?></span></div>
            <?php if ($directadmin_configured): ?>
            <div class="info">🔑 Database password will be auto-generated and shown after installation.</div>
            <?php else: ?>
            <div class="info">DirectAdmin credentials are empty, so database creation and wp-config.php generation will be skipped.</div>
            <?php endif; ?>
        </div>
        ...
        <div class="loading" id="loading">
            ...
            <p>Installing WordPress and plugins...<br>
...
function updatePreviews() {
    ...
    var daEnabled = <?php echo $directadmin_configured ? 'true' : 'false'; ?>;
